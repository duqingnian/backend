...
import { Label, MenuItem, Button, InputGroup, Drawer, Classes, SegmentedControl, HTMLSelect, Dialog, DialogBody, DialogFooter, FormGroup } from "@blueprintjs/core";
...
class Payin extends Component {
    ...
    show_detail(request_token) {
        let that = this
        Ajax("order", { action: "detail", bundle: "PAYIN", request_token: request_token }, function (json) {
            if (0 === json.code) {
                that.setState({
    ...
    load_refund = () => {
        ...
        Ajax("order", { action: "load_refund", request_token: that.state.selected_order.request_token }, function (json) {
            if (0 === json.code) {
                that.setState({
                    refund_info: json.refund_info
                })
            }
            else {
                message.error(json.msg);
    ...
    render() {
        return (<div>
            <div className="PageBreadcrumb" style={{ display: 'inline-block' }}>
                <Breadcrumb items={[{ text: '管理面板', key: 'adminPannel' }, { text: '订单', key: 'order' }, { text: '代收', key: 'order-index' }]} onReduceData={() => undefined} maxDisplayedItems={5} />
            </div>
                                            ...
                                            <td className='cp'>{
                                                'GENERATED' === order.status
                                                    ...
                                                    : ('SUCCESS' === order.status
                                                        ? <Button intent="Success" minimal={false} onClick={() => { this.sync_channel_status(order) }}>{order.status}</Button>
                                                        : <Button intent="Danger" minimal={false} onClick={() => { this.sync_channel_status(order) }}>{order.status}</Button>)
                                            }</td>
                                            <td className='cp'>
                                                <table style={{ margin: '0 auto' }}>
                                                    <tbody>
            ...
            >
                ...
                <DialogFooter actions={<Button intent="primary" text="关闭" onClick={() => {
                    this.setState({
                        merchant_notify_result: {
                            is_open: false,
                            merchant: { id: 0, name: '' },
                ...
                <div className={Classes.DRAWER_BODY}>
                    {
                        '' !== this.state.form.request_token ? <div className={Classes.DIALOG_BODY}>
                            <Pivot aria-label="订单信息" defaultSelectedKey="basic" onLinkClick={(pivot_item) => {
                                if (".$order_refund" === pivot_item.key) {
                                    this.load_refund()
                                }
                                else {
                                    //
                                            ...
                                            ("undefined" !== typeof (this.state.selected_order.process_list) && this.state.selected_order.process_list.length > 0) && this.state.selected_order.process_list.map((process) => {
                                                return <table className='model mb15' key={"process_" + process.bundle}>
                                                    <tbody>
                                                        ...
                                                        <tr><td style={{ textAlign: 'left' }}>{process.data}</td></tr>
                                                    </tbody>
                                                </table>
                                            })
                                        }
                                ...
                                <PivotItem headerText="退款" key="order_refund">
                                    <div className='pd15' style={{ 'background': '#aecff12b' }}>
                                        {
                                            "SUCCESS" === this.state.selected_order.status ?
                                                <Button
                                                    intent="Danger"
                                                    text="退款"
                                                    onClick={() => {
                                                        this.setState({ popover: { refund: true } })
                                                    }}
                                                />
                                                : <p>仅成功状态订单才可以发起退款，当前订单状态：{this.state.selected_order.status}</p>
                                        }
                                        <table className='model mt15'>
                                            <tr><td style={{ width: '120px', textAlign: 'right' }}>退款状态:</td><td>{this.state.refund_info.status}</td></tr>
                                            <tr><td style={{ textAlign: 'right' }}>通道单号:</td><td>{this.state.selected_order.cno}</td></tr>
                                            <tr><td style={{ textAlign: 'right' }}>平台单号:</td><td>{this.state.selected_order.pno}</td></tr>
                                            <tr><td style={{ textAlign: 'right' }}>商户单号:</td><td>{this.state.selected_order.mno}</td></tr>
                                            <tr><td style={{ textAlign: 'right' }}>退款金额:</td><td>{this.state.refund_info.amount}</td></tr>
                                            <tr><td style={{ textAlign: 'right' }}>发起时间:</td><td>{this.state.refund_info.created_time}</td></tr>
                                            <tr><td style={{ textAlign: 'right' }}>发起IP:</td><td>{this.state.refund_info.created_ip}</td></tr>
                                            <tr><td style={{ textAlign: 'right' }}>回调时间:</td><td>{this.state.refund_info.notify_time}</td></tr>
                                            <tr><td style={{ textAlign: 'right' }}>回调IP:</td><td>{this.state.refund_info.notify_ip}</td></tr>
                                            <tr><td style={{ textAlign: 'right' }}>回调数据:</td><td>{this.state.refund_info.notify_data}</td></tr>
                                            <tr><td style={{ textAlign: 'right' }}>快照金额:</td><td>{this.state.refund_info.balance_snapshot}</td></tr>
                                            <tr><td style={{ textAlign: 'right' }}>退款之前金额:</td><td>{this.state.refund_info.amount_before}</td></tr>
                                            <tr><td style={{ textAlign: 'right' }}>退款之后金额:</td><td>{this.state.refund_info.amount_after}</td></tr>
                                            <tr><td style={{ textAlign: 'right' }}>退款时间:</td><td>{this.state.refund_info.refund_time}</td></tr>
                                            <tr><td style={{ textAlign: 'right' }}>日志编号:</td><td>{this.state.refund_info.logid}</td></tr>
                                        </table>
                                    </div>
                                </PivotItem>
                            </Pivot>
            ...
            <Dialog
                title={"退款确认 - " + this.state.selected_order.id + " - " + this.state.selected_order.pno}
                icon="info-sign"
                onClose={() => { this.setState({ popover: { refund: false } }) }}
                isOpen={this.state.popover.refund}>
                <DialogBody>
                    <div>确定退款 {this.state.selected_order.amount}-{this.state.selected_order.currency} 吗?</div>
                </DialogBody>
                <DialogFooter actions={<>
                    <Button onClick={() => { this.setState({ popover: { refund: false } }) }}>关闭</Button>
                    <Button intent="Danger" text={"确认退款" + this.state.selected_order.amount + this.state.selected_order.currency} onClick={this.dispatch_refund} />
                </>
                } />
            </Dialog>
    ...
    dispatch_refund = () => {
        let that = this
        Ajax("order", { action: "refund", request_token: that.state.selected_order.request_token }, function (json) {
            if (0 === parseInt(json.code)) {
                that.setState({ popover: { refund: false } }, function () {
    ...
    send_notify = (order,exchange, tip) => {
        ...
        Modal.confirm({
            ...
            cancelText: '取消',
            onOk() {
                Ajax("order", { action: "send_notify", bundle: "PAYIN", exchange: exchange, request_token: order.request_token }, function (json) {
                    if (0 === json.code) {
                        that.setState({
